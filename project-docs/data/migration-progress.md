...
- **P7-T12** (Blazor parity sweep): Product detail page at `/item/:itemId` with "Log in to purchase" / "Add to shopping bag" flow; catalog card becomes a clickable Link with no inline Add-to-Cart button; checkout simplified to address-only with JWT-claim defaults (matches `PopulateFormWithDefaultInfo()`); Cart rewritten to 3-column flex layout (Products / Quantity / Total) with per-row Update button and right-side summary sidebar (cart icon + qty badge, total, "Check out", "Continue shopping"); per-page document titles via `useDocumentTitle` hook ({Name} | AdventureWorks pattern)

End-to-end verified after Phase 7: SPA loads at http://localhost:8080 with AdventureWorks visual brand, hero banner, real product images; catalog cards open the product detail page; anonymous users get "Log in to purchase" → OIDC redirect; logged-in users add to cart, see the shopping bag with line-item Update / summary sidebar, check out with JWT-prefilled address, order #92 advanced Submitted → Paid via the saga; per-page titles ("Shopping Bag | AdventureWorks", "Checkout | AdventureWorks", etc.) match the Blazor reference.
